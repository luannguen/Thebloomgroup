...
async function seed() {
    ...
    try {
        ...
        // 2. Add to Navigation (Footer - Về Thebloomgroup)
        const { rows: footerNav } = await client.query('SELECT id FROM navigation WHERE label = $1', ['Về Thebloomgroup']);
        if (footerNav.length > 0) {
            ...
            if (navExists.length === 0) {
                ...
                console.log('Recruitment link added to "Về Thebloomgroup" footer navigation');
            } else {
                console.log('Recruitment link already exists in footer');
            }
        } else {
            console.log('Footer parent "Về Thebloomgroup" not found');
        }
